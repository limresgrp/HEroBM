...
@compile_mode("script")
class HierarchicalBackmappingGradModule(GraphModuleMixin, torch.nn.Module):
    # saved params
    num_layers: int
    field: str
    inv_out_field: str
    ...
    def __init__(
        ...
        self.eq_out_length_field = eq_out_length_field
        ...
        for layer_idx in range(num_layers):
            ...
            if layer_idx == self.num_layers - 1:
                ...
                ir_out = o3.Irreps([ir for ir in env_embed_irreps if ir.ir.l == 0])
        ...
        inv_out_field = final_latent_irreps if not self.readout_features else self.inv_out_irreps
        eq_out_field =  final_latent_irreps if not self.readout_features else o3.Irreps(f"{self.final_readout_eq.out_features}x0e")
        self.irreps_out.update(
            {
                self.inv_out_field: inv_out_field,
                self.eq_out_field:   eq_out_field,
                self.eq_out_length_field: eq_out_field,
            }
        )
    ...
    def forward(self, data: AtomicDataDict.Type) -> AtomicDataDict.Type:
        ...
        if self.readout_features:
            data[self.inv_out_field] = self.final_readout_inv(final_latent_input)
            data[self.eq_out_field] =  self.final_readout_eq(final_latent_input)
            data[self.eq_out_length_field] =  self.final_readout_eq_length(final_latent_input)
