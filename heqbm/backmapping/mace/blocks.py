...
class NonLinearNodeEmbeddingBlock(torch.nn.Module):
    def __init__(
        self, irreps_in: o3.Irreps, irreps_out: o3.Irreps, MLP_irreps: List[o3.Irreps], gate: Optional[Callable]
    ):
        super().__init__()
        self.hidden_irreps = [irreps_in, *MLP_irreps, irreps_out]
        self.linears = torch.nn.ModuleList()
        self.non_linearities = torch.nn.ModuleList()
        for i, (ir_in, ir_out) in enumerate(zip(self.hidden_irreps[:-1], self.hidden_irreps[1:])):
            self.linears.append(o3.Linear(irreps_in=ir_in, irreps_out=ir_out))
            if i < len(self.hidden_irreps) - 2:
                self.non_linearities.append(nn.Activation(irreps_in=ir_out, acts=[gate]))
        self.in_irreps = self.hidden_irreps[0]
        self.out_irreps = self.hidden_irreps[-1]

    def forward(self, x: torch.Tensor) -> torch.Tensor:  # [n_nodes, irreps]  # [..., ]
        for linear, non_linearity in zip(self.linears, self.non_linearities):
            x = non_linearity(linear(x))
        return self.linears[-1](x)  # [n_nodes, 1]
